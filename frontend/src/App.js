...
// ─── Avatar system ────────────────────────────────────────────────────────────
// Each avatar has: a background colour, a border colour (lighter tint), and an emoji
const AVATARS = [
...
// Avatar component — consistent across entire app
// Shows: coloured background circle + border + emoji
// Dead: everything goes greyscale, opacity drops
function Avatar({ num, size = 10, highlight = false, dead = false, showBorder = true }) {
  const idx  = Math.max(0, (num - 1) % AVATARS.length);
  const av   = AVATARS[idx];
  const px   = size * 4; // size unit → pixels (Tailwind size-unit = 4px each)
  const fontSize = px * 0.52;
  ...
  return (
    <div
      style={{
        ...
        border: showBorder ? `2.5px solid ${dead ? '#52525b' : (highlight ? '#ffffff' : av.border)}` : 'none',
        boxShadow: highlight && !dead ? `0 0 0 3px rgba(255,255,255,0.25)` : 'none',
        borderRadius: '50%',
        display: 'flex',
        alignItems: 'center',
...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      if (saved) {
        ...
        } catch { sessionStorage.removeItem('killerSession'); }
      }
    });
    ...
    // Rejoin failed — clear session, go to main menu with message
    socket.on('rejoinFailed', msg => {
      sessionStorage.removeItem('killerSession');
      setMyInfo(null);
      setRoom(null);
      setError(msg || 'Session expired. Please rejoin the game.');
      setLoading(false);
    });

    socket.on('roomUpdate', r => {
      // Sync timer & maxTimer from server on every full update
      setTimer(r.timer || 0);
      ...
      setRoom(prev => {
        if (prev?.status !== r.status) {
          if (r.status === 'ROLE_REVEAL') {
            setShowRole(false);  // start hidden so player taps to reveal
    ...
    // Lightweight tick — only updates timer numbers
  ...
  const handleJoin = () => {
    if (!name.trim()) return setError('Enter your name first.');
    if (code.trim().length < 6) return setError('Enter the 6-digit game code.');
    setLoading(true); setError('');
    ...
    return (
      <Screen>
        <div className="text-center py-10">
          ...
          <h1 className="text-3xl font-black text-yellow-400 mb-1">Game Over</h1>
          <p className="text-zinc-500 mb-8">Final standings</p>
          ...
          <button className="btn-primary" onClick={() => { setGameEnded(null); setError(''); }}>
            Back to Menu
          </button>
        </div>
      </Screen>
      ...
      <Screen>
        <div className="flex flex-col items-center justify-center min-h-screen py-12">
          ...
          <div className="w-full max-w-xs space-y-3">
            {error && <div className="bg-red-900/40 border border-red-800 text-red-300 text-sm rounded-2xl p-3 text-center">{error}</div>}
            ...
            <button className="btn-primary w-full" onClick={handleHost} disabled={loading}>
              {loading ? <Spinner size={5} /> : '🎮  Host a Game'}
            </button>

            <Divider label="or join" />

            <input className="input-field text-center tracking-widest text-xl font-bold"
              placeholder="Game code" value={code} inputMode="numeric"
              onChange={e => { setCode(e.target.value.replace(/\D/g, '').slice(0, 6)); setError(''); }}
              onKeyDown={e => e.key === 'Enter' && handleJoin()} maxLength={6} />

            <button className="btn-secondary w-full" onClick={handleJoin} disabled={loading}>
              {loading ? <Spinner size={5} /> : '🚪  Join Game'}
            </button>
          </div>

          {!connected && (
            <div className="mt-8 flex items-center gap-2 text-zinc-600 text-xs">
              <Spinner size={4} /><span>Connecting to server…</span>
            </div>
          )}
        </div>
      </Screen>
  ...
  // ─── Waiting for roomUpdate after join ────────────────────────────────────
  if (!me) {
    return (
      <Screen>
        <div className="flex flex-col items-center justify-center min-h-screen gap-4">
          <Spinner /><p className="text-zinc-500">Joining game…</p>
        </div>
      </Screen>
    );
  }
        ...
        <div className="py-6 space-y-5">
          ...
          <div className="text-center">
            ...
            <div className="inline-block bg-zinc-900 border border-zinc-700 rounded-3xl px-8 py-5">
              <span className="text-5xl font-black tracking-[0.2em] text-white">{room.id}</span>
            </div>
                ...
                <div key={p.id} className="flex items-center gap-3">
                  ...
                  {isHost && p.id !== me.id && (
                    <button onClick={() => emit('removePlayer', { roomId, playerId: p.id })}
                      className="text-zinc-600 hover:text-red-500 transition px-1 text-lg leading-none">✕</button>
                  )}
                </div>
              ))}
            </div>
          ...
          {isHost && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
              ...
              >
                ...
                <span className="text-zinc-500 text-xs">{showConfig ? '▲ hide' : '▼ show'}</span>
              </button>
              {showConfig && (
                <div className="px-4 pb-5 space-y-4 border-t border-zinc-800 pt-4">
                  <ConfigSlider label="Role reveal time" value={pendingConfig.roleRevealTime} min={8} max={30} step={2}
                    unit="s" onChange={v => applyConfig('roleRevealTime', v)} />
                  <ConfigSlider label="Night action time" value={pendingConfig.questionTime} min={10} max={60} step={5}
                    unit="s" onChange={v => applyConfig('questionTime', v)} />
                  <ConfigSlider label="Discussion time" value={pendingConfig.discussionTime} min={30} max={300} step={15}
                    unit="s" format={fmtTime} onChange={v => applyConfig('discussionTime', v)} />
                  <ConfigSlider label="Voting time" value={pendingConfig.votingTime} min={15} max={90} step={5}
                    unit="s" onChange={v => applyConfig('votingTime', v)} />
                  <ConfigSlider label="Result display" value={pendingConfig.resultDuration} min={4} max={15} step={1}
                    unit="s" onChange={v => applyConfig('resultDuration', v)} />
  ...
  function applyConfig(key, val) {
    const next = { ...pendingConfig, [key]: val };
      ...
      <GameScreen room={room} me={me} isHost={isHost} emit={emit} roomId={roomId} timer={timer} maxTimer={maxTimer}>
        <div className="flex flex-col items-center py-6 gap-6">
          ...
          <div
            ...
            style={{ borderColor: showRole ? info.border : '#3f3f46', backgroundColor: showRole ? info.bg : 'transparent' }}
            className="w-full max-w-xs rounded-3xl border-2 p-8 text-center cursor-pointer transition-all duration-300 active:scale-95"
          >
            {showRole ? (
              <>
          ...
          <div className="flex items-center gap-2 text-zinc-600 text-xs">
            <TimerBar timer={timer} maxTimer={maxTimer} color={PHASE_COLORS.ROLE_REVEAL} />
            <span className="shrink-0">{timer}s</span>
          </div>
        </div>
      </GameScreen>
    );
        ...
        <div className="py-4 space-y-5">
          ...
          {myNightSubmitted ? (
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                {targets.map(p => (
                  <PlayerCard key={p.id} player={p} onClick={() => handleNightAction(p.id)} />
                ))}
                {me.role === 'DETECTIVE' && (
      ...
      <GameScreen room={room} me={me} isHost={isHost} emit={emit} roomId={roomId} timer={timer} maxTimer={maxTimer}>
        {me.alive ? (
          <div className="py-4 space-y-5">
            ...
            {myVote ? <SubmittedBox label="Vote cast!" sub="Waiting for others…" /> : (
              <div className="grid grid-cols-2 gap-3">
    ...
    return (
      <GameScreen room={room} me={me} isHost={isHost} emit={emit} roomId={roomId} timer={timer} maxTimer={maxTimer}>
        <div className="py-4 text-center space-y-5">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
            <p className="text-zinc-500 text-xs uppercase tracking-widest mb-3">Session Scores</p>
...
// ─── PlayerCard — consistent avatar tap target ────────────────────────────────
function PlayerCard({ player, onClick }) {
  const av = AVATARS[Math.max(0, (player.avatar - 1) % AVATARS.length)];
  return (
    <button
      onClick={onClick}
      style={{ borderColor: '#3f3f46' }}
      className="flex flex-col items-center p-4 bg-zinc-900 rounded-2xl border hover:border-zinc-500 active:scale-95 transition gap-2"
    >
    ...
    <Screen>
      ...
      <div className="sticky top-0 z-10 bg-zinc-950/95 backdrop-blur-sm pt-3 pb-3 mb-1">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-sm font-bold text-zinc-200">{STATUS_LABEL[room.status] || room.status}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {isHost && (
              <button onClick={() => setShowControls(v => !v)}
                className="bg-zinc-800 rounded-xl p-2 text-zinc-400 hover:text-white transition text-lg leading-none">
                ⚙
              </button>
            )}
          </div>
      ...
      {showControls && isHost && (
        <div className="fixed inset-0 bg-black/85 z-50 flex flex-col items-center justify-center gap-3 p-6">
          ...
          <button onClick={() => { emit('pauseGame',  { roomId }); setShowControls(false); }} className="btn-secondary w-full max-w-xs">⏸  Pause Game</button>
          <button onClick={() => { emit('resumeGame', { roomId }); setShowControls(false); }} className="btn-primary  w-full max-w-xs">▶  Resume Game</button>
...
function Tag({ children, color = 'zinc' }) {
  const colors = {
    yellow: 'bg-yellow-900/30 text-yellow-500 border-yellow-800/50',
    zinc:   'bg-zinc-800 text-zinc-500 border-zinc-700',
    red:    'bg-red-900/30 text-red-400 border-red-800/50',
  };
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${colors[color] || colors.zinc}`}>
      {children}
    </span>
  );
}
...
function SubmittedBox({ label = 'Action submitted!', sub = 'Waiting for others…' }) {
  return (
...
function SpectatorScreen({ logs }) {
  return (
    <div className="text-center py-12 px-4 space-y-3">
      ...
      {logs && <p className="text-zinc-500 text-sm bg-zinc-900 border border-zinc-800 rounded-xl p-3">{logs}</p>}
    </div>
  );
}
